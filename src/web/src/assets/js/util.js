...
path = []
function findPath(vertex, visitedVertices) {
  path.push(vertex)
  let vertexKey = vertex.getKey();
  let newVertex =  visitedVertices[vertexKey];
  if(newVertex == null) {
    console.log(path);
    return path;
  }
  findPath(newVertex, visitedVertices);

}

function Test() {
  const vertexA = new GraphVertex('Noida Electronic City');
  const vertexB = new GraphVertex('B');
  const vertexC = new GraphVertex('C');
  const vertexD = new GraphVertex('D');
  const vertexE = new GraphVertex('E');
  const vertexF = new GraphVertex('Patel Chowk');
  ...
  graph
   ...
   let x =   graph.getVertexByKey('Noida Electronic City');
  const { distances, previousVertices } = dijkstra(graph, x);
  // console.log("distanace", distances);
  // console.log("vertices", previousVertices);
  let y =   graph.getVertexByKey('Patel Chowk');
  let z = findPath(y, previousVertices);
